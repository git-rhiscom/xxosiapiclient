...
public class CargaInicialMaestroDeProductosDAO implements ICargaInicialMaestroDeProductosDAO {
	...
	public List<ProductoXXOSI> leerProductosXXOSI() throws PersistenceExceptionInventario {
		...
		listadoProductosNuevos = (List<ProductoXXOSI>)session.createQuery("select v from ProductoXXOSI v where estado = 'N'").list();
	...
	@Override
	public void crearProductos(List<ProductoXXOSI> listadoProductos) throws PersistenceExceptionInventario {
		...
		while( iterador.hasNext() ) {
			...
			producto.setAlternativeItemId1(productoXXOSI.getAlternativeItemId1());	
			producto.setAlternativeItemId2(productoXXOSI.getAlternativeItemId2());
			producto.setAlternativeItemId3(productoXXOSI.getAlternativeItemId3());
			producto.setAlternativeItemId4(productoXXOSI.getAlternativeItemId4());
			producto.setPriceEntryRequiredFlag(productoXXOSI.getPriceEntryRequiredFlag());
			...
			producto.setAuthorizedForSaleFlag(productoXXOSI.getAuthorizedForSaleFlag());
			producto.setConsignationFlag(productoXXOSI.getConsignationFlag());
			producto.setTaxExemptCode(productoXXOSI.getTaxExemptCode());
			producto.setWeightOrUnitCountCode(productoXXOSI.getWeightOrUnitCountCode());
	...
	@Override
	public void actualizarProductos(List<ProductoXXOSI> listadoProductos) throws PersistenceExceptionInventario {
		...
		// Default data source connection parameters.		
				...
				while( iterador.hasNext() ) {
					productoXXOSI = new ProductoXXOSI();
					productoXXOSI = (ProductoXXOSI) iterador.next();
					Query consulta = session.createQuery("update Producto set itemName = :itemName, itemDescription = :itemDescription" + 
					...
					consulta.setParameter("itemDescription", productoXXOSI.getUnitOfMeasureCode());
					consulta.setParameter("alternativeItemId1", productoXXOSI.getAlternativeItemId1());
					consulta.setParameter("alternativeItemId2", productoXXOSI.getAlternativeItemId2());
					consulta.setParameter("alternativeItemId3", productoXXOSI.getAlternativeItemId3());
					consulta.setParameter("alternativeItemId4", productoXXOSI.getAlternativeItemId4());
					consulta.setParameter("priceEntryRequiredFlag", productoXXOSI.getPriceEntryRequiredFlag());
					consulta.setParameter("discountableFlag", productoXXOSI.getDiscountableFlag());
					consulta.setParameter("promotionableFlag", productoXXOSI.getPromotionableFlag());
					consulta.setParameter("authorizedForSaleFlag", productoXXOSI.getAuthorizedForSaleFlag());
					consulta.setParameter("consignationFlag", productoXXOSI.getConsignationFlag());
					consulta.setParameter("taxExemptCode", productoXXOSI.getTaxExemptCode());
					consulta.setParameter("weightOrUnitCountCode", productoXXOSI.getWeightOrUnitCountCode());
					consulta.setParameter("unitOfMeasureCode", productoXXOSI.getUnitOfMeasureCode());
	...
	@Override
	public void actualizarEstadoProductosXXOSI(List<ProductoXXOSI> listadoProductos) throws PersistenceExceptionInventario {

		// Default data source connection parameters.		
		Session session = HibernateUtil.getSessionFactoryXXOSI().getCurrentSession();
		session.beginTransaction();
		//String itemId = "";
		...
		while( iterador.hasNext() ) {
			...
			Query consulta = session.createQuery("update ProductoXXOSI set estado = 'T' where itemId = '" + productoXXOSI.getItemId() + "'");
			int result = consulta.executeUpdate();
	...
	@Override
	public List<ProductoXXOSI> validadorDeExistentes(List<ProductoXXOSI> listadoProductos) throws PersistenceExceptionInventario {
		...
		if (stringIdProductos.length() > 0){
			...
			listadoIdProductos = (List<String>)session.createQuery("select itemId from Producto where itemId in (" + stringIdProductos + ") ").list();
			iterador = listadoProductos.listIterator();
			while( iterador.hasNext() ) {
				productoXXOSI = new ProductoXXOSI();
				productoXXOSI = (ProductoXXOSI) iterador.next();
				for (int i=0; i<=listadoIdProductos.size()-1;i++){	
					System.out.println(listadoIdProductos.get(i)+"="+productoXXOSI.getItemId());
					if(listadoIdProductos.get(i).equals(productoXXOSI.getItemId())){
						System.out.println(" YEAAAAASS");
